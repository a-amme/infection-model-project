...
def Plot_Intracellular(variable, color=None, title=None, xlabel='Time (hrs)', ylabel=None, mean = False):
    ...
    epsilon = 1E-20
    if not mean:
        plt.plot(f['Time'],np.percentile(np.log10(V+epsilon), 50, axis =1),color=color,linewidth = 3.0)
        lower_bound = np.percentile(np.log10(V+epsilon), 5, axis = 1)
        upper_bound =  np.percentile(np.log10(V+epsilon), 95, axis = 1)
    else:
        plt.plot(T, np.mean(np.log10(V + epsilon), axis=1), color=color, linewidth=3.0)
